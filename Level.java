...
//---------------------------------------------------------------------------------
  ...
  class GraphicsPanel extends JPanel {
    ...
    public void paintComponent(Graphics g) {
      ...
      for (int i = 0; i < level.energyGun.numBullets; i++) {
        if (level.energyGun.bulletVisible[i]) {
  ...
  class MyKeyListener implements KeyListener {
    ...
    public void keyPressed(KeyEvent e) {
      ...
      //Switch to basic gun
      if (key == KeyEvent.VK_1 && level.curGun != level.basicGun) {
        level.curGun = level.basicGun;
      }
      //Switch to energy gun
      else if (key == KeyEvent.VK_2 && level.curGun != level.energyGun) {
        level.curGun = level.energyGun;
      }
      //Switch to machine gun
      else if (key == KeyEvent.VK_3 && level.curGun != level.machineGun) {
